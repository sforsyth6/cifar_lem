...
class NCEFunction(Function):
    @staticmethod
    def forward(self, x, y, memory, idx, params, updated_weights):
        K = int(params[0].item())
        T = params[1].item()
        Z = params[2].item()
        ...
        self.save_for_backward(x, memory, y, weight, out, params, updated_weights)

        return out

    @staticmethod
    def backward(self, gradOutput):
        x, memory, y, weight, out, params, updated_weights = self.saved_tensors
        K = int(params[0].item())
        T = params[1].item()
        Z = params[2].item()
        momentum = params[3].item()
        ...
        memory.index_copy_(0, y, updated_weights)
...
class NCEAverage(nn.Module):
    ...
    def forward(self, x, y):
        ...
        momentum = self.params[3]
        weight = torch.index_select(self.memory, 0, idx.view(-1))
        weight.resize_(batchSize, self.K+1, inputSize) 
        weight_pos = weight.select(1, 0).resize_as_(x)
        weight_pos.mul_(momentum)
        weight_pos.add_(torch.mul(x.data, 1-momentum))
        w_norm = weight_pos.pow(2).sum(1, keepdim=True).pow(0.5)
        updated_weights = weight_pos.div(w_norm)

        #calculate prox. reg.C
        dif = updated_weights - weight_pos
        dif_norm = torch.norm(dif, dim=-1)**2
        lam = 50
        reg = lam*dif_norm

        self.reg = reg

        out = NCEFunction.apply(x, y, self.memory, idx, self.params, updated_weights)
